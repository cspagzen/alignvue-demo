...
loadConversations() {
  ...
  // Update the header to include conversation controls
  renderConversationControls() {
    const headerControls = document.querySelector('.vuesense-modal-actions');
    if (!headerControls) return;
    
    // Add conversation menu button
    const menuBtn = document.createElement('button');
    menuBtn.className = 'vuesense-icon-btn vuesense-conversation-menu-btn';
    menuBtn.innerHTML = `
    ...
    const settingsBtn = headerControls.querySelector('.vuesense-settings-btn');
    if (settingsBtn) {
      headerControls.insertBefore(menuBtn, settingsBtn);
    }
  }
  ...
  toggleConversationMenu() {
    ...
    menu.innerHTML = `
      <div class="vuesense-menu-section">
        ...
        <button class="vuesense-menu-item danger" onclick="window.vuesenseModal.clearConversation()">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          ...
          ${this.conversations.slice(0, 5).map(conv => `
            <div class="vuesense-conversation-item ${conv.id === this.currentConversationId ? 'active' : ''}">
              <button class="vuesense-conversation-load" onclick="window.vuesenseModal.loadConversation(${conv.id})">
                <div class="vuesense-conversation-title">${conv.title}</div>
