...
// ===============================================================================
// PHASE 3: INTERACTIVE TEAM HEALTH MODAL SYSTEM (Clean JavaScript Version)
// Add these functions to your existing script.js (after the permanent integration code)
// ===============================================================================

// Updated dimension cell class function to handle 4 states
function getDimensionCellClass(dimensionValue) {
    switch(dimensionValue) {
        case 'Healthy': return 'healthy';
        case 'At Risk': return 'at-risk'; 
...
function getDimensionTooltip(dimensionName, dimensionValue) {
    const tooltips = {
        'capacity': 'Capacity - Workload & Resources',
        'skillset': 'Skillset - Technical Capabilities', 
        'vision': 'Vision - Clarity & Direction',
        'support': 'Support - Tools & Organizational Backing',
        'teamwork': 'Team Cohesion - Collaboration & Communication',
        'autonomy': 'Autonomy - Decision-making Authority'
    };
    
    const baseTooltip = tooltips[dimensionName] || dimensionName;
    const stateText = dimensionValue || 'Not Set';
    return `${baseTooltip}: ${stateText} (Click to update)`;
}

// Main function to open the team health modal
function openTeamHealthModal(teamName) {
    console.log(`Opening team health modal for: ${teamName}`);
    ...
    if (!teamData) {
        ...
        alert(`Team "${teamName}" not found. Please refresh the page.`);
        return;
    }
    ...
    title.innerHTML = `
        <div class="flex items-center gap-3">
            ...
            Team Health: ${teamName}
        </div>
    `;
    
    content.innerHTML = `
        <form id="team-health-form" class="team-health-form" onsubmit="handleTeamHealthUpdate(event, '${teamName}')">
            
            <!-- Current Status Display -->
            <div class="health-status-display mb-6">
                <h3 class="text-sm font-medium mb-3" style="color: var(--text-primary);">Current Team Health Matrix</h3>
                <div class="dimensions-grid-2x3 pointer-events-none" style="margin: 0 auto; width: fit-content;">
                    <div class="grid-column">
                        <div class="dimension-cell ${getDimensionCellClass(teamData.capacity)}" title="${getDimensionTooltip('capacity', teamData.capacity)}">C</div>
                        <div class="dimension-cell ${getDimensionCellClass(teamData.support)}" title="${getDimensionTooltip('support', teamData.support)}">Su</div>
                    </div>
                    <div class="grid-column">
                        <div class="dimension-cell ${getDimensionCellClass(teamData.skillset)}" title="${getDimensionTooltip('skillset', teamData.skillset)}">S</div>
                        <div class="dimension-cell ${getDimensionCellClass(teamData.teamwork)}" title="${getDimensionTooltip('teamwork', teamData.teamwork)}">T</div>
                    </div>
                    <div class="grid-column">
                        <div class="dimension-cell ${getDimensionCellClass(teamData.vision)}" title="${getDimensionTooltip('vision', teamData.vision)}">V</div>
                        <div class="dimension-cell ${getDimensionCellClass(teamData.autonomy)}" title="${getDimensionTooltip('autonomy', teamData.autonomy)}">A</div>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                ...
                <!-- Capacity -->
                <div>
                    <label for="capacity" class="block text-sm font-medium mb-2" style="color: var(--text-primary);">
                        <span class="flex items-center gap-2">
                            <span>Capacity</span>
                            <span class="text-xs px-2 py-1 rounded" style="background: var(--bg-quaternary); color: var(--text-tertiary);">Workload & Resources</span>
                        </span>
                    </label>
                    <select id="capacity" class="team-health-select w-full px-3 py-2 border rounded-md" style="border-color: var(--border-primary); background: var(--bg-secondary); color: var(--text-primary);">
                        <option value="">Not Set</option>
                        <option value="Healthy" ${teamData.capacity === 'Healthy' ? 'selected' : ''}>Healthy</option>
                        <option value="At Risk" ${teamData.capacity === 'At Risk' ? 'selected' : ''}>At Risk</option>
                        <option value="Critical" ${teamData.capacity === 'Critical' ? 'selected' : ''}>Critical</option>
                    </select>
                </div>
                
                <!-- Skillset -->
                <div>
                    <label for="skillset" class="block text-sm font-medium mb-2" style="color: var(--text-primary);">
                        <span class="flex items-center gap-2">
                            <span>Skillset</span>
                            <span class="text-xs px-2 py-1 rounded" style="background: var(--bg-quaternary); color: var(--text-tertiary);">Technical Capabilities</span>
                        </span>
                    </label>
                    <select id="skillset" class="team-health-select w-full px-3 py-2 border rounded-md" style="border-color: var(--border-primary); background: var(--bg-secondary); color: var(--text-primary);">
                        <option value="">Not Set</option>
                        <option value="Healthy" ${teamData.skillset === 'Healthy' ? 'selected' : ''}>Healthy</option>
                        <option value="At Risk" ${teamData.skillset === 'At Risk' ? 'selected' : ''}>At Risk</option>
                        <option value="Critical" ${teamData.skillset === 'Critical' ? 'selected' : ''}>Critical</option>
                    </select>
                </div>
                
                <!-- Vision -->
                <div>
                    <label for="vision" class="block text-sm font-medium mb-2" style="color: var(--text-primary);">
                        <span class="flex items-center gap-2">
                            <span>Vision</span>
                            <span class="text-xs px-2 py-1 rounded" style="background: var(--bg-quaternary); color: var(--text-tertiary);">Clarity & Direction</span>
                        </span>
                    </label>
                    <select id="vision" class="team-health-select w-full px-3 py-2 border rounded-md" style="border-color: var(--border-primary); background: var(--bg-secondary); color: var(--text-primary);">
                        <option value="">Not Set</option>
                        <option value="Healthy" ${teamData.vision === 'Healthy' ? 'selected' : ''}>Healthy</option>
                        <option value="At Risk" ${teamData.vision === 'At Risk' ? 'selected' : ''}>At Risk</option>
                        <option value="Critical" ${teamData.vision === 'Critical' ? 'selected' : ''}>Critical</option>
                    </select>
                </div>
                
                <!-- Support -->
                <div>
                    <label for="support" class="block text-sm font-medium mb-2" style="color: var(--text-primary);">
                        <span class="flex items-center gap-2">
                            <span>Support</span>
                            <span class="text-xs px-2 py-1 rounded" style="background: var(--bg-quaternary); color: var(--text-tertiary);">Tools & Organizational Backing</span>
                        </span>
                    </label>
                    <select id="support" class="team-health-select w-full px-3 py-2 border rounded-md" style="border-color: var(--border-primary); background: var(--bg-secondary); color: var(--text-primary);">
                        <option value="">Not Set</option>
                        <option value="Healthy" ${teamData.support === 'Healthy' ? 'selected' : ''}>Healthy</option>
                        <option value="At Risk" ${teamData.support === 'At Risk' ? 'selected' : ''}>At Risk</option>
                        <option value="Critical" ${teamData.support === 'Critical' ? 'selected' : ''}>Critical</option>
                    </select>
                </div>
                
                <!-- Team Cohesion -->
                <div>
                    <label for="teamwork" class="block text-sm font-medium mb-2" style="color: var(--text-primary);">
                        <span class="flex items-center gap-2">
                            <span>Team Cohesion</span>
                            <span class="text-xs px-2 py-1 rounded" style="background: var(--bg-quaternary); color: var(--text-tertiary);">Collaboration & Communication</span>
                        </span>
                    </label>
                    <select id="teamwork" class="team-health-select w-full px-3 py-2 border rounded-md" style="border-color: var(--border-primary); background: var(--bg-secondary); color: var(--text-primary);">
                        <option value="">Not Set</option>
                        <option value="Healthy" ${teamData.teamwork === 'Healthy' ? 'selected' : ''}>Healthy</option>
                        <option value="At Risk" ${teamData.teamwork === 'At Risk' ? 'selected' : ''}>At Risk</option>
                        <option value="Critical" ${teamData.teamwork === 'Critical' ? 'selected' : ''}>Critical</option>
                    </select>
                </div>
                
                <!-- Autonomy -->
                <div>
                    <label for="autonomy" class="block text-sm font-medium mb-2" style="color: var(--text-primary);">
                        <span class="flex items-center gap-2">
                            <span>Autonomy</span>
                            <span class="text-xs px-2 py-1 rounded" style="background: var(--bg-quaternary); color: var(--text-tertiary);">Decision-making Authority</span>
                        </span>
                    </label>
                    <select id="autonomy" class="team-health-select w-full px-3 py-2 border rounded-md" style="border-color: var(--border-primary); background: var(--bg-secondary); color: var(--text-primary);">
                        <option value="">Not Set</option>
                        <option value="Healthy" ${teamData.autonomy === 'Healthy' ? 'selected' : ''}>Healthy</option>
                        <option value="At Risk" ${teamData.autonomy === 'At Risk' ? 'selected' : ''}>At Risk</option>
                        <option value="Critical" ${teamData.autonomy === 'Critical' ? 'selected' : ''}>Critical</option>
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                ...
                <!-- Utilization -->
                <div>
                    <label for="utilization" class="block text-sm font-medium mb-2" style="color: var(--text-primary);">
                        Team Utilization (%)
                    </label>
                    <input 
                        type="number" 
                        ...
                        min="0" 
                        max="100" 
                        value="${teamData.jira?.utilization || 0}"
                ...
                <div>
                    ...
                    <input 
                        type="text" 
                        value="${teamData.jira?.key || 'Not linked'}"
                        readonly
                        class="w-full px-3 py-2 border rounded-md opacity-75" 
                        style="border-color: var(--border-primary); background: var(--bg-tertiary); color: var(--text-secondary);"
                    />
                </div>
            </div>
            ...
            <!-- Comments -->
            <div class="mb-6">
                <label for="comments" class="block text-sm font-medium mb-2" style="color: var(--text-primary);">
                ...
                <textarea 
                    ...
                    class="w-full px-3 py-2 border rounded-md" 
                    style="border-color: var(--border-primary); background: var(--bg-secondary); color: var(--text-primary);"
                    placeholder="Add notes about the team's current health status..."
                >${teamData.jira?.comments || ''}</textarea>
            </div>
            
            <!-- Sync Status (Hidden by default) -->
            <div id="sync-status" class="sync-status mb-4">
                <div class="sync-icon">
                    <svg id="sync-icon" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="animate-spin">
                ...
                <span id="sync-message">Syncing changes to Jira...</span>
            </div>
...
// Handle team health form submission
async function handleTeamHealthUpdate(event, teamName) {
    event.preventDefault();
    
    console.log(`Saving team health changes for: ${teamName}`);
    
    const form = document.getElementById('team-health-form');
    const submitButton = document.getElementById('submit-team-health');
    ...
    try {
        ...
        const formData = {
            ...
            utilization: parseInt(document.getElementById('utilization').value) || 0,
        ...
        const teamData = boardData.teams[teamName];
        if (teamData) {
            teamData.capacity = formData.capacity;
            teamData.skillset = formData.skillset;
            teamData.vision = formData.vision;
            teamData.support = formData.support;
            teamData.teamwork = formData.teamwork;
            teamData.autonomy = formData.autonomy;
        ...
        if (teamData?.jira?.key) {
            await updateTeamHealthInJira(teamData.jira.key, formData);
        } else {
            console.warn(`No Jira key found for team: ${teamName}`);
        }
        
        // Show success
        showSyncSuccess();
        
        // Update UI if needed (refresh team cards)
        if (typeof updateAllCards === 'function') {
            updateAllCards();
        }
...
// Update team health data in Jira
async function updateTeamHealthInJira(issueKey, data) {
    console.log(`Updating Jira issue ${issueKey} with:`, data);
    
    // Map form values to Jira custom field format
    const fields = {
        ...
        customfield_10264: data.utilization, // Utilization (number field)
        customfield_10263: data.comments // Comments
    };
...
function showSyncSuccess() {
    ...
    syncIcon.style.color = 'var(--accent-green)';
    syncMessage.textContent = 'Changes saved successfully!';
}
...
function showSyncError(message) {
    ...
    syncIcon.innerHTML = '✗';
    syncIcon.classList.remove('animate-spin');
    syncIcon.style.color = 'var(--accent-red)';
    syncMessage.textContent = `Error: ${message}`;
}

console.log('Team Health Modal system loaded');
console.log('Available functions:');
console.log('   - openTeamHealthModal(teamName)');
console.log('   - handleTeamHealthUpdate(event, teamName)');
console.log('   - updateTeamHealthInJira(issueKey, data)');

// ===============================================================================
// END OF TEAM HEALTH MODAL SYSTEM
// ===============================================================================
